...
metricRes$ID <- metricRes$ID %>% as.factor()
levels(metricRes$ID) <- c('Non-diabetic|6126', 'Non-diabetic|6134', 
                          'Onset|6228','Onset|6362',  'Long-duration|6089', 
                          'Long-duration|6180')
# plot metrics
...
We note that, according to the estimated confidence bands, there is no difference in the $G$ function between non-diabetic and long-duration T1D samples, but a difference between non-diabetic and onset T1D according to the model summary. This difference is non-significant according to the point wise confidence bands in the coefficients plots. The point wise confidence bands are a limitation of this method and could be improved with either bootstrapping or continuous confidence bands [@liebl2023fast]. Nevertheless, we get an indication that this difference is probably due to interactions at larger radii ($r>40$) . Thus, we see not only that a spatial difference in co-localisation of $\alpha$ and cytotoxic T cells is statistically significant but also at which spatial scale this difference occurs.
